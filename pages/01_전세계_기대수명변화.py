...
time_series = df.groupby('year').lex.mean().reset_index()
fig = px.line(time_series, x='year', y='lex', labels={'year':'연도','lex':'평균 기대수명(세)'}, title="전 세계 평균 기대수명 변화")
st.plotly_chart(fig, use_container_width=True)

with st.expander("🔍 사용 설명서 설명 보기"):
    st.write("- 슬라이더로 연도를 변경하세요.\n- 마우스 오버로 세부값 확인.")
with st.expander("💡 학생 토론 질문"):
    st.markdown("1. 19세기 말 기대수명이 낮았던 이유는?\n2. 주요 전염병이 미친 영향은?")
with st.expander("📚 교육적 함의 및 확장 활동"):
    st.write("- 역사적 보건 정책 분석.\n- 기대수명 격차 해소 정책 제안.")
